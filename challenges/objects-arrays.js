...
function Dinosaur (attrs) {
  ...
  this.weight = attrs.weight; // weight is in kg
  this.length = attrs.length; // length is in m
...
const universities = graduates.sort((a, b) => {
  ...
  else if (a.university < b.university) return -1;
  return 0;
});
...
const uni = graduates.filter ((graduate) => {
  if (graduate.university.includes("Uni")) return graduate.university;
})
console.log(uni);
...
console.log(animalNames); // just noticed it said .forEach... why? This way seems easier. 

zooAnimals.forEach((animal) => {
  let newAnimalNames = [];
  newAnimalNames.push(animal.animal_name + " " + animal.scientific_name);
